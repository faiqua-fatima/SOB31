...
def check_winner(game):
    ...
    return 0 #faiqua fatima changed return winner to return 0
...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player #faiqua fatima removed +1
    return game
...
def switch_player(player):
    if player == 1:#"=" changed to "==" by faiqua fatima
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: #faiuqa fatima added colon
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row,column) #faiqua fatima added column as an argument
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game)#faiqua fatima removed "#" so that it operates
